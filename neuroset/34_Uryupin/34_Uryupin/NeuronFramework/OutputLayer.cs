...
namespace _34_Uryupin.src
{
    class OutputLayer : Layer
    {
        ...
        public override void Recognize(NeuralNetwork net, Layer nextLayer)
        {
            ...
            // Нормализация выходных значений нейронов (получаем вероятностное распределение)
            for (int i = 0; i < numOfNeurons; i++)
            {
        ...
        // метод выполнения обратного распространения ошибки
        public override double[] BackwardPass(double[] errors)
        {
            ...
            for (int i = 0; i < numOfNeurons; i++) // Вычисление коррекции синаптических весов 
            {
                ...
                {
                    double deltaW;// переменная для хранения изменения веса
